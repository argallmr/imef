...
from imef.efield.model_creation.Neural_Networks import NN_parameters
...
def train_NN(args):
    ...
    # Do I have to recreate this for each model?
    if adam:
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
    else:
        optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate)

    # container = NN_parameters(NN_layout, batch_size, learning_rate, epochs, kfold, undersample, loss_fn, optimizer)
    ...
    for train_filename in train_filename_list:
        ...
        one_file_inputs, one_file_targets = NN_func.get_NN_inputs(one_file, use_values=values_to_use, undersample=undersample)
    ...
    # The actual training
    # Note that if k-fold is not done the final test error will be 0, as nothing is tested
    if kfold:
        ...
        for train_index, test_index in kf.split(total_inputs):
            ...
            for t in range(epochs):
                print(f"Epoch {t + 1}\n-------------------------------")
                NN_func.train_loop(train_dataloader, model, loss_fn, optimizer, device)

            test_error = NN_func.test_loop(test_dataloader, model, loss_fn, device)
            final_test_error += test_error

        final_test_error = final_test_error / (kf.get_n_splits(total_inputs))

        print("Done!")
        output = NN_func.output_error(values_to_use, model.parameters(), number_of_layers, final_test_error,
                              file_to_output_to='test_errors.txt')
...
def main():
    ...
    parser.add_argument('-k', '--kfold',
                        default=None,
                        type=int,
                        help='If the user wants to run K-fold validation, type -k and the number of folds to have (eg. -k 10). Note that the model trained on the last fold will be saved, and not a model trained on all the data',
                        )
    ...
    parser.add_argument('-ru', '--random_undersampling',
                        default=None,
                        type=float,
                        help='Randomly undersample the data file given, such that the quiet time data and storm time data are made more balanced. '
                             'User should input a number that represents the ratio of storm data to quiet data. E.g. -ru 2 would use a dataset with 2x as much quiet data than storm data.',
                        )
