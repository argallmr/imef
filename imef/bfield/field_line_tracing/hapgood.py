...
def dipole_igrf2latlon(g10, g11, h11):
    ...
    geo_lon = np.arctan2(h11, g11)
    
    # Hapgood (1997) -- the corrigendum to Hapgood (1992) -- says to use
    # arctan instead of arcsine here
    geo_lat = 90.0 - np.rad2deg(np.arctan((g11*np.cos(geo_lon)
                                           + h11*np.sin(geo_lon)
                                           ) / g10))
    return geo_lat, np.rad2deg(geo_lon)
...
def dipole_inclination(time):
    ...
    -------
    psi : float
        Inclination angle between z-GSM and z-GSe in degrees
    '''
