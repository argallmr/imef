...
def plot_Kp(data, data2, nbins, nbins2, mode):
    ...
    for row in range(nbins):
        for col in range(3):
            ...
            if mode=='polar': # Doesnt work for polar, since it goes 3 times but only 2 coords
                ...
                im = ax.pcolormesh(phi, r, data[string].loc[:, :, list[col]], cmap='seismic', shading='auto', vmin=-.5, vmax=.5) # These values may need to be changed
                new_string = string[9:-5].split('_')
                if row == 2:
                    ax.set_title('E' + list[col] + ' Kp: [' + new_string[0] + ',' + new_string[2] + ']')
                else:
    ...
    for plot_number in range (nbins):
        ...
        im = ax2.pcolormesh(phi, r, count_data, cmap='OrRd', shading='auto', vmax='1200')  # These values may need to be changed
        ...
        for Kp_value in range (8):
            ...
            if Kp_value==7:
                ...
                for counter3 in range(nL):
                    ...
                    list.append((data2[string][counter3, :, component].values+data2[string2][counter3, :, component].values).mean())
                    ax2.plot(data['iL'].values[counter3] + 4.5, data2[string][counter3, :, component].values.mean(), color=colors[Kp_value], marker="o", markersize=3)
...
def main():
    data = xr.open_dataset('1_year_Kp3.nc')
    data2 = xr.open_dataset('1_year_Kp9.nc')
    nbins=3.0
    nbins2=9.0
    mode='cartesian'
