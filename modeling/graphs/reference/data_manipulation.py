...
def average_data_3d(new_data, created_file): # Add Weighted Average Stuff
    ...
    for L_counter in range(0, 30):
        for MLT_counter in range(0, 24):
            for MLAT_counter in range(-90, 90, 10):
                # Selecting data that will go in each bin
                ...
                new_row = pd.DataFrame(
                    [[L_counter + .5, MLT_counter + .5, MLAT_counter + 5, EDI_X_avg, EDI_Y_avg, EDI_Z_avg]],
...
def average_data_2d(new_data, created_file):
    '''
        Parameters:
            new_data ():
            created_file (boolean): Whether the desired output file has been created or not
        Returns:
            df:
        '''
    # Where the average values will be stored
    averaged_edi_data = pd.DataFrame(columns=['L', 'MLT', 'er', 'etheta', 'ephi', '#ofValues'])
    ...
    for L_counter in range(0, 30):
        for MLT_counter in range(0, 24):
            ...
            # Need to covert the whole dataframe to polar coordinates, so that I can do standard deviation correctly

            EDI_r_avg = np.sqrt(
                EDI_X_avg ** 2 + EDI_Y_avg ** 2 + EDI_Z_avg ** 2)  # Same units, but may want to figure out what that is
            ...
            # Inputting newly found averages into dataframe
            new_row = pd.DataFrame([[L_counter + .5, MLT_counter + .5, EDI_r_avg, EDI_theta_avg, EDI_phi_avg, len(the_values_to_be_averaged)]],
                                   columns=['L', 'MLT', 'er', 'etheta', 'ephi', '#ofValues'])
...
def combine_new_data(new_data, created_file):
    if not created_file:
        ...
        for counter in range(len(file_data)):
            ...
            if not isnan(file_data.loc[counter, 'er']) and not isnan(new_data.loc[counter, 'er']):

                # Determine number of values to conduct weighted average
                file_size = file_data.loc[counter, '#ofValues']
                new_size = new_data.loc[counter, '#ofValues']

                # Weighted average
                file_data.loc[counter] = (new_size * new_data.loc[counter] + file_size * file_data.loc[counter]) / (file_size + new_size)

                # Doing average this way messes up the size column. So save correct column numbers prior to average,
                # Fix the new_data's size to what is was, and
                # combine the new_data and file_data's sizes together since they are now averaged
                file_data.loc[counter, '#ofValues'] = file_size + new_size
                new_data.loc[counter, '#ofValues'] = new_size

            # If there is data at a bin that is not already in the existing datafile,
            # supplant the NaN in the file with the new data
            elif isnan(file_data.loc[counter, 'er']) and not isnan(new_data.loc[counter, 'er']):
...
def cart2polar(pos_cart):
    '''
    Rotate cartesian position coordinates to polar coordinates
    '''
    RE = 6371 # km
    r = (np.sqrt(np.sum(pos_cart ** 2, axis=1)) / RE)
